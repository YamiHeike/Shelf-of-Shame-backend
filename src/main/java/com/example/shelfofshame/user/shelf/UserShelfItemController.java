...
public class UserShelfItemController {
    ...
    @PostMapping("/add")
    public ResponseEntity<UserShelfItemDto> addNewBookToShelf(@RequestBody @Valid @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Data Transfer Object with ISBN-10 and book metadata") AddBookToShelfDto addBookToShelfDto, Principal principal) {
    ...
    @PostMapping("/add-new")
    public ResponseEntity<UserShelfItemDto> createAndBookAddToShelf(@RequestBody @Valid @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Data Transfer Object with new book and shelf item details") AddNewBookToShelfDto addNewBookToShelfDto, Principal principal) {
        User user = authenticatedUserProvider.getCurrentUser(principal);
